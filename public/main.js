...
const chooseBot = (id) => {
    ...
    if (playerDuo.length === 2) {
        ...
        rollbar.info('Alert "You can only choose two bots!" returned when attempting to select more than one bot')
    }
}
...
const getPlayerStats = () => {
    axios.get('http://localhost:4000/api/player')
        .then(({data: {wins, losses}}) => {
            ...
            rollbar.info('wins and losses are being displayed after each duel')
        })
}
    ...
    axios.get('http://localhost:4000/api/robots')
        .then(({data}) => {
            ...
            data.forEach(bot => {
                ...
                rollbar.warning('all bots not showing up when clicked.')
            })
        })
}
